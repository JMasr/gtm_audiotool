...
class FeatureExtractor:
    ...
    def __init__(self, arguments: dict):
        self.args = arguments  # input arguments dictionary
        self.audio_path = None  # path to the audio file
        self.resampling_rate = self.args.get('resampling_rate')  # resampling rate
    ...
    def _read_audio(self, audio_file_path: str) -> (np.ndarray, int):
        """
    ...
    @staticmethod
    def compute_sad(sig: torch.Tensor, fs: int, threshold: float = 0.0001, sad_start_end_sil_length: int = 100,
                    sad_margin_length: int = 50) -> np.ndarray:
        """
        Compute a threshold based sound activity detection
        :param sig: matrix of audio signal
        :param fs: sampling rate
        :param threshold:  threshold for sound activity detection
        :param sad_start_end_sil_length:  length of leading/trailing silence
        :param sad_margin_length: margin around active samples
        :return: a matrix with 1 for active samples and 0 for inactive samples
        """
        ...
        # Compute the sound activity detection
        ...
        # Apply the leading/trailing silence
        sad[0, 0:sad_start_end_sil_length] = 0
        sad[0, -sad_start_end_sil_length:] = 0
        return sad

    def _do_feature_extraction(self, s: np.array, sr: int) -> np.ndarray:
        """
        The code above implements the following steps for feature extraction:
        ...
        :return: the features extracted matrix of the audio signal
        """
        # Initialize the feature extractor
        feature_extractor = None

        if self.args['feature_type'] == 'MelSpec':
            feature_extractor = self.feature_transform(s)

        if self.args['feature_type'] == 'logMelSpec':
            feature_extractor = self.feature_transform(s)
            feature_extractor = torchaudio.functional.amplitude_to_DB(feature_extractor, multiplier=10, amin=1e-10,
                                                                      db_multiplier=0)
        ...
        if 'ComParE_2016' in self.args['feature_type']:
            #
            s = s[None, :]
            feature_extractor = self.feature_transform.process_signal(s, sr)
            ...
            if self.args['feature_type'] == 'ComParE_2016_llds':
                feature_subset['subset'] = list(feature_extractor.columns)

            feature_extractor = feature_extractor[feature_subset['subset']].to_numpy()
            feature_extractor = np.nan_to_num(feature_extractor)
            feature_extractor = torch.from_numpy(feature_extractor).T
        ...
        if 'Spafe_' in self.args['feature_type']:
            ...
            if self.args['feature_type'] in ['Spafe_mfcc', 'Spafe_imfcc', 'Spafe_gfcc', 'Spafe_lfcc', 'Spafe_msrcc',
                ...
                feature_extractor = self.feature_transform(s, sr,
                ...
                feature_extractor = self.feature_transform(s, sr, num_ceps=int(self.args.get('n_mfcc')),
                                                           low_freq=int(self.args.get('f_min')),
                                                           high_freq=int(sr // 2),
                                                           nfilts=int(self.args.get('n_mels')),
                                                           nfft=nfft)
                ...
                feature_extractor = self.feature_transform(s, sr, order=int(self.args.get('plp_order')))
                if isinstance(feature_extractor, tuple):
                    feature_extractor = feature_extractor[0]
        ...
        if self.args.get('compute_deltas', False):
            feats_deltas = torchaudio.functional.compute_deltas(feature_extractor)
            feature_extractor = torch.cat((feature_extractor, feats_deltas), dim=0)

            if self.args.get('compute_deltas_deltas', False):
                feats_deltas_deltas = torchaudio.functional.compute_deltas(feats_deltas)
                feature_extractor = torch.cat((feature_extractor, feats_deltas_deltas), dim=0)

        # Apply mean normalization
        if self.args.get('apply_mean_norm', False):
            feature_extractor = feature_extractor - torch.mean(feature_extractor, dim=0)

        # Apply variance normalization
        if self.args.get('apply_var_norm', False):
            feature_extractor = feature_extractor / torch.std(feature_extractor, dim=0)

        # Add descriptors from OpenSmile
        if self.args.get('extra_features', False) and 'ComParE_2016' not in self.args['feature_type']:
            ...
            # Extract features
            feature_extractor_descriptors = extra_transform.process_signal(s, sr)
            feature_extractor_descriptors = feature_extractor_descriptors[feature_subset['subset']].to_numpy()
            feature_extractor_descriptors = np.nan_to_num(feature_extractor_descriptors)
            feature_extractor_descriptors = torch.from_numpy(feature_extractor_descriptors).T

            # Concatenate the features
            common_shape = min(feature_extractor.shape[1], feature_extractor_descriptors.shape[1])
            feature_extractor = torch.cat(
                (feature_extractor[:, :common_shape], feature_extractor_descriptors[:, :common_shape]), dim=0)

        return feature_extractor.T
